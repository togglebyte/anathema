...
use crate::templates::error::{ParseErrorKind, Result};
use crate::templates::expressions::Equality;
...
pub(super) fn eval(expr: Expr, strings: &Strings) -> Result<Expression, ParseErrorKind> {
    let output = match expr {
        ...
        Expr::Binary { op, lhs, rhs } => match op {
            ...
            op @ (Operator::DotDot | Operator::DotDotDot) => {
                let (start, end) = (eval(*lhs, strings)?.into(), eval(*rhs, strings)?.into());
                Expression::Range {
                    start,
                    end,
                    inclusive: op == Operator::DotDotDot,
                }
            }
