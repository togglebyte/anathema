...
mod test {
    ...
    fn test_stack(h_edge: HorzEdge, v_edge: VertEdge, expected: &str) {
        ...
        position.add_child(Text::with_text(fields::POSITION_X).into_container(NodeId::anon()));
        stack.add_child(border.into_container(NodeId::anon()));
        stack.add_child(position.into_container(NodeId::anon()));
