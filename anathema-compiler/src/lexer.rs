...
use super::components::TemplateSource;
use super::error::{ParseError, ParseErrorKind, Result};
use super::strings::Strings;
use super::token::{Kind, Operator, Token, Value};
...
mod test {
    ...
    fn error_kind(input: &'static str) -> ParseErrorKind {
        ...
        match err.kind {
            ErrorKind::ParseError(err) => err.kind,
            ErrorKind::CircularDependency
            | ErrorKind::InvalidStatement(_)
            | ErrorKind::MissingComponent(_)
            | ErrorKind::EmptyTemplate
            | ErrorKind::EmptyBody
            | ErrorKind::GlobalAlreadyAssigned(_)
            | ErrorKind::Io(_) => panic!("invalid error"),
        }
    }
