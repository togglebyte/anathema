...
impl<V: State> List<V> {
    ...
    pub fn get(&self, index: usize) -> Option<&Value<V>> {
        self.inner.get(index)
    }

    pub fn get_mut(&mut self, index: usize) -> Option<&mut Value<V>> {
    ...
    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut Value<V>> {
        self.inner.iter_mut()
    }
...
impl<V: State> Default for List<V> {
    fn default() -> Self {
        Self { inner: VecDeque::new() }
    }
}
...
impl<V: State> Value<List<V>> {
    ...
    pub fn extract_if<F>(&mut self, mut f: F) -> Vec<Value<V>>
    where
        F: FnMut(&Value<V>) -> bool,
    {
    ...
    pub fn push_front(&mut self, value: impl Into<Value<V>>) {
    ...
    pub fn insert(&mut self, index: usize, value: impl Into<Value<V>>) {
        let value = value.into();
    ...
    pub fn remove(&mut self, index: usize) -> Option<Value<V>> {
    ...
    pub fn pop_front(&mut self) -> Option<Value<V>> {
    ...
    pub fn pop_back(&mut self) -> Option<Value<V>> {
...
impl<V: State> AnyList for List<V> {
...
impl<V> FromIterator<V> for Value<List<V>>
where
    V: State,
    Value<V>: From<V>,
{
...
mod test {
    ...
    use super::*;
    use crate::testing::mock_value_index;
    use crate::value::{Changes, ValueIndex};

    fn changes() -> Vec<(ValueIndex, Change)> {
    ...
    fn notify_insert() {
        ...
        list.reference().subscribe(mock_value_index());
        list.push_back(1);
