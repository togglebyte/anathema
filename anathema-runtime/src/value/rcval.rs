...
pub fn changed_many(change: Change, keys: impl Iterator<Item = ValueIndex>) {
    CHANGES.with_borrow_mut(|changes| keys.for_each(|key| changes.push((key, change))));
}
...
pub struct Subs(RcRefCell<Vec<ValueIndex>>);
...
impl Subs {
    pub(crate) fn changed(&mut self, change: Change) {
        changed_many(change, self.0.borrow_mut().iter().copied());
    }
...
impl<'a, V> ValueMut<'a, V> {
    pub fn sub(&mut self, sub: ValueIndex) {
...
impl AnonValue {
    pub fn subscribe(&self, key: ValueIndex) {
