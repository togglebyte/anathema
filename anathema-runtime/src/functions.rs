...
pub struct Function {
    inner: Box<dyn for<'bp> Fn(&[TemplateValue<'bp>]) -> TemplateValue<'bp>>,
}

impl Function {
    pub(crate) fn invoke<'bp>(&self, args: &[TemplateValue<'bp>]) -> TemplateValue<'bp> {
        (self.inner)(args)
    }
}
...
where
    T: 'static,
    T: for<'bp> Fn(&[TemplateValue<'bp>]) -> TemplateValue<'bp>,
{
