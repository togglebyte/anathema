...
pub use rcval::{AnonValue, Value, ValueMut, ValueRef};

pub use self::list::List;
pub use self::map::Map;
pub use self::maybe::Maybe;
use crate::states::State;
pub use crate::value::changes::Change;
...
pub trait SubKey: PartialEq + Eq + Copy + 'static {
    fn changed_many(change: Change, keys: impl Iterator<Item = Self>);
}

impl<K: SubKey, V: Default + State<K>> Default for Value<K, V> {
    fn default() -> Self {
        Self::new(V::default())
    }
}

impl<K: SubKey, V: State<K>> From<V> for Value<K, V> {
